...
struct Shader
{
	...
	{
		return glUniform1i(glGetUniformLocation(id, name.c_str()), (int)value), *this;
	}

	Shader& setInt(const std::string &name, int value)
	{
		return glUniform1i(glGetUniformLocation(id, name.c_str()), value), *this; 
	}

	Shader& setFloat(const std::string &name, float value)
	{ 
		return glUniform1f(glGetUniformLocation(id, name.c_str()), value), *this; 
	}

	Shader& setMatrix(const std::string &name, mat4 value)
	{
		return glUniformMatrix4fv(glGetUniformLocation(id, name.c_str()), 1, GL_FALSE, &value[0][0]), *this;
	}

	Shader& setVec3(const std::string &name, vec3 value)
	{
		return glUniform3f(glGetUniformLocation(id, name.c_str()), value.x, value.y, value.z), *this;
	}
};
